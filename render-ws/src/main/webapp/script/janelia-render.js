...
var JaneliaScriptUtilities = function() {
     ...
     * @param stackInfo.stats.channelNames
     */
    JaneliaRenderServiceDataUI.prototype.getStackSummaryHtml = function(ownerUrl, stackInfo) {
        ...
        function getNdvizUrl(host,shortbaseUrl,owner, project,stack) {
            return `http://${JSON.stringify(host)}/render/${JSON.stringify(shortbaseUrl)}/${JSON.stringify(owner)}/${JSON.stringify(project)}/${JSON.stringify(stack)}/?blend=additive`;
        }
    
        if (this.isNdvizHostDefined()) {
            let baseNDVIZUrl = getNdvizUrl(this.ndvizHost,this.shortbaseUrl,stackId.owner,stackId.project,stackId.stack);
            if (isSSL){
                baseNDVIZUrl += `&ssl=true`;
            }
            if (stackInfo.stats.hasOwnProperty('channelNames')){
                const channels = stackInfo.stats.channelNames;
                for (let i=0; i<channels.length; i++) {
                    linksHtml = linksHtml + 
                        `<a target="_blank" href="${baseNDVIZUrl}&channel=${JSON.stringify(channels[i])}">NdViz: ${JSON.stringify(channels[i])} </a>`;
                }
            } else {
                linksHtml = linksHtml + `<a target="_blank" href="${baseNDVIZUrl}">NdViz</a>`;
            }
        }
